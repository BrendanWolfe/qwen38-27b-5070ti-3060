...
# then profiles the real steady-state footprint. Gotcha 51 records the evidence.
#
set -e
...
for n in $(seq 1 "$ATTEMPTS"); do
  ...
  if [ "$rc" -ne 0 ] && grep -q 'estimated maximum model length' "$LOG" \
     && [ "$n" -lt "$ATTEMPTS" ]; then
    echo "start_qwen_solo: compile scratch was charged to KV sizing (attempt $n/$ATTEMPTS); relaunching with the warmed compile cache. See gotcha 51." >&2
    continue
  fi
  exit "$rc"
done
