...
Last synced with upstream at **`c32be47` on 2026-09-02**. This brings in eight
new vLLM patches, KVarN correctness and first-request warmup work, bounded-RAM
checkpoint preparation, and reproducible benchmark tools. The mixed-GPU
profiles adopt automatic correctness/warmup fixes, expose the promising
BF16-prefill, int4 multi-query and Mamba-retention paths as opt-ins, and keep
their measured PP partitions, cache budgets and metric defaults. Upstream's
one-card/TP throughput claims are not copied onto unequal cards over PCIe x1.
